...
from beamng_sim.self_driving.edit_distance_polyline import iterative_levenshtein
from beamng_sim.self_driving.simulation_data import SimulationDataRecord

from beamng_sim.code_pipeline.test_analysis import direction_coverage, max_curvature
from beamng_sim.code_pipeline.test_analysis import sd_steering, mean_lateral_position, max_lateral_position
...
class OOBAnalyzer:
    ...
    def _load_road_points(self, result_folder):
        road_points = []
        road_x
        for subdir, dirs, files in os.walk(result_folder, followlinks=False):
            ...
            for sample_file in sorted(
                ...
                try:
                    ...
                    road_points = road_data
                    if test_outcome == "FAIL":
                        road_points.append(road_data)
                except Exception as e:
                    # Using logger.exception generates logs that are a bit worrysome
                    self.logger.warning(f"Failed to process points for test {sample_file}. Cause: {e}")
                    continue
    ...
    def _compute_dist_matrix(self):
        ...
        dist_matrix = np.zeros((len(self.oobs), len(self.oobs)))
        i = 0
        j = i + 1
        ...
        '''
        try:
            clustering = AgglomerativeClustering(n_clusters=None, linkage='average', metric='precomputed', compute_full_tree=True, distance_threshold=5)
            clusters= clustering.fit_predict(dist_matrix)
        except Exception as e:
             print(e)
        #plt.close()
        #print(f"Number of clusters = {1+np.amax(clusters)}")
        #plt.close()
    ...
    def _compute_sparseness(self):
        ...
        clustering = AgglomerativeClustering(n_clusters=None, linkage='single', metric='precomputed', compute_full_tree=True, distance_threshold=5)
        clusters= clustering.fit_predict(dist_matrix)
        #plt.close()
        #print(f"Number of clusters = {1+np.amax(clusters)}")
        #plt.close()
